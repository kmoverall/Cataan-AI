...
void BoardState::GenerateNew()
{
	...
	for (auto& it : m_map) {
		for (int i = 0; i < 6; i++)
		{
			...
			{
				m_edge_list.push_back( new Edge(m_edge_list.size()) );
				it.second->edges[i] = m_edge_list.back();

				//Add a reference to the edge to hex that shares that edge
				if (GetHex(it.first + HexDirections::VECTORS[i]) != NULL) 
				{
			...
			{
				m_corner_list.push_back( new Corner(m_corner_list.size()) );
				...
				if (GetHex(it.first + HexDirections::VECTORS[(i + 1) % 6]) != NULL)
				{
